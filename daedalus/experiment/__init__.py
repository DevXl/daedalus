...
class BaseExperiment:
    """
    The most basic experiment functionalities
    """
    EXP_TYPE = "BASIC"
    ...
    def __init__(self, path: str, name: str):
        ...
        subjects_metadata_file = self.HOME_DIR / "data" / "participants.tsv"
        self.subjects_metadata = pd.read_csv(subjects_metadata_file, sep="\t")
    ...
    def initiate_session(self):
        ...
        session_subject = self._choose_subject()
        session = 0
        if _subject == 'new':
    ...
    def _choose_subject(self):
        ...
        _initials = self.subjects_metadata.name.tolist()  # subjects' initials
        _sids = self.subjects_metadata.sID.tolist()  # subjects' student ids
        ...
        choose_subj_dlg = gui.Dlg(title=self.NAME, labelButtonOK="Next",
                                  labelButtonCancel="Register new participant")
        ...
        while not selected:
            ...
            if choose_subj_dlg.OK:
                ...
                if choose_subj_info[0] != 'Select':
                    _session_subject = self.subjects_metadata[self.subjects_metadata.name == choose_subj_info[0]]
                    selected = True
                elif choose_subj_info[1] != 'Select':
                    _session_subject = self.subjects_metadata[self.subjects_metadata.sID == choose_subj_info[1]]
                    selected = True
            else:
                # user wants to register a new subject
                selected = True
                _session_subject = self._register_subject()
    ...
    def initialize_experiment(self):
        ...
        # data directory
        init_log.write("Load\n====")
        logging.info("Reading the DATA directory...")
        ...
        # log directory
        logging.info("Reading the LOG directory...")
    ...
    def choose_subject(self):
        """show a gui to choose or register new subject"""
    ...
    def get_session_info(self):
        ...
        session_info = {}
        session_dlg = gui.Dlg(title="Session information")
        session_dlg.addFixedField(self.SUB["name"])
        session_dlg.addFixedField(self.SUB["sID"])

        for param in self.EXP.values():
            session_dlg.addField(param["description"], choices=list(param["init_vals"]))
        ...
        if session_dlg.OK:
            ...
            k = 2
            for key in self.EXP.key():
                self.INFO[key] = session_info[k]
                k += 1
        else:
            sys.exit()
    ...
    def register_subject(self):
        ...
        if new_subj_dlg.OK:
            ...
            new_subj_info["name"] = user_info[1]
